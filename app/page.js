...
export default function Home(){
 ...
 const y=month.getFullYear(),m=month.getMonth(), first=new Date(y,m,1), blanks=(first.getDay()+6)%7,days=new Date(y,m+1,0).getDate();
 return <main>
  ...
  {view==="week"?<><Nav title={`${fmt(monday)} – ${fmt(sunday)}`} prev={()=>setWeek(add(week,-7))} next={()=>setWeek(add(week,7))}/><Cards items={weekEvents}/></>:<>
   ...
   <div className="calendar"><div className="dows">{["월","화","수","목","금","토","일"].map(x=><b key={x}>{x}</b>)}</div><div className="grid">{Array.from({length:blanks}).map((_,i)=><div key={"b"+i}/>) }{Array.from({length:days},(_,i)=>i+1).map(d=>{const es=filtered.filter(e=>e.date===key(new Date(y,m,d)));return <div className="cell" key={d}><strong>{d}</strong>{es.map((e,i)=><span key={i} style={{color:color(e.people[0]),background:color(e.people[0])+"20"}}>{short(e.school)}({e.people.map(p=>p[0]).join("·")})</span>)}</div>})}</div></div>
  </>}
